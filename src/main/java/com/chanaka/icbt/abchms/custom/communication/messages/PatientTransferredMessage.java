...
public class PatientTransferredMessage extends CommunicationMessage {
	public PatientTransferredMessage(Admission admission, MessageTypes messageType) {
		this.title = Texts.PATIENT_TRANSFERRED_TO_HOSPITAL_TITLE;
		String message = "";
		switch (messageType) {
		case SMS:
			message = Texts.PATIENT_TRANSFERRED_TO_HOSPITAL_SMS;
			this.receiver = admission.getPatient().getContactPersonMobile();
			break;
		case EMAIL:
			message = Texts.PATIENT_TRANSFERRED_TO_HOSPITAL_EMAIL;
