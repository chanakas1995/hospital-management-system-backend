...
public class PatientDischargedMessage extends CommunicationMessage  {
	public PatientDischargedMessage(Admission admission, MessageTypes messageType) {
		this.title = Texts.PATIENT_DISCHARGED_TO_HOSPITAL_TITLE;
		String message = "";
		switch (messageType) {
		case SMS:
			message = Texts.PATIENT_DISCHARGED_TO_HOSPITAL_SMS;
			this.receiver = admission.getPatient().getContactPersonMobile();
			break;
		case EMAIL:
			message = Texts.PATIENT_DISCHARGED_TO_HOSPITAL_EMAIL;
